...
*/
void all_init_particles(int num_particles, double* positions, double* masses)
{
  int    i;
...
#if 0
    positions[2*i+0] = ((rand() % max_resolution)- (max_resolution/2))*2.0 / max_resolution;
    positions[2*i+1] = ((rand() % max_resolution)- (max_resolution/2))*2.0 / max_resolution;
   
#else
    positions[2*i+0] = i*2.0/num_particles - 1.0;
    positions[2*i+1] = 0.0;
    
#endif
    masses[i] = 1.0 + (num_particles+i)/total_particle;
     //insert_particle(particle, root);
  }
}

void local_init_particles_v(int nparticles,double* velocities,double* positions)
{
     for (int i = 0; i < nparticles; i++) {
         #if 0
         velocities[2*i+0] = positions[2*i+1];
         velocities[2*i+1] = positions[2*i+0];
         #else
         velocities[2*i+0] = 0.0;
         velocities[2*i+1] = positions[2*i+0];
         #endif
     }
}
